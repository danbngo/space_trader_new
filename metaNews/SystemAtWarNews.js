class SystemAtWarNews extends News {
    ...
    isValid() {
        ...
        const warCount = planets.reduce((count, planet) => {
            const hostilePlanets = Array.from(planet.c.relationships.entries()).filter(([targetPlanet, relationship]) => relationship == RELATIONSHIP_TYPES.WAR)
    ...
    static getWarsToSpread(numWarsToStart = 1) {
        ...
        for (const planet of planetsRandom1) {
            for (const targetPlanet of planetsRandom2) {
                if (planet.c.relationships.get(targetPlanet) == RELATIONSHIP_TYPES.TENSE) {
