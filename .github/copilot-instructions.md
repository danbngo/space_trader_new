...
- UI replace pattern: menus call `showElement()` which wipes `#game-container`. To create non-modal overlays, attach to DOM manually via `createElement()`.
...
- Add a menu: create a function in `menus/*.js` that builds content with `createElement()` and calls `showModal(title, content, buttons)`; follow `menus/titleMenu.js` and `menus/planetMenu.js` for examples.
